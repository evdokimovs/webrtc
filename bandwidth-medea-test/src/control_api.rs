...
impl ControlClient {
    ...
    ///
    /// - if connection with server failed.
    pub async fn try_get(&mut self, uri: &str) -> Result<Option<proto::Element>, proto::Error> {
        let room = vec![uri.to_string()];
        let get_room_request = proto::IdRequest { fid: room };
        ...
        Ok(resp.elements.remove(&uri.to_string()))
    }
    ...
    /// - if connection with server failed.
    pub async fn create(&mut self, req: proto::CreateRequest) -> HashMap<String, String> {
    ...
    /// Applies the provided `proto::Element` and returns its sids.
    ///
    /// # Panics
    ///
    /// - If the returned [`CreateResponse`] has error.
    /// - If the connection with server failed.
    pub async fn apply(&mut self, req: proto::ApplyRequest) -> HashMap<String, String> {
        let resp = self.0.apply(req).await.unwrap().into_inner();
        if let Some(e) = resp.error {
            panic!("{e:?}");
        }
    ...
    /// # Panics
    ///
    /// If connection with server fails.
    pub async fn healthz(&mut self, ping: u32) -> u32 {
        self.0
            .healthz(proto::Ping { nonce: ping })
            .await
            .unwrap()
            .into_inner()
            .nonce
...
impl Room {
    pub fn build_request<T: Into<String>>(self, uri: T) -> proto::CreateRequest {
        let members = self
            .members
            .into_iter()
            .map(|(id, member)| {
    ...
    pub fn build_apply_request<T: Into<String>>(self, uri: T) -> proto::ApplyRequest {
        let members = self
            .members
            .into_iter()
            .map(|(id, member)| {
        ...
        proto::ApplyRequest {
            parent_fid: uri.into(),
            el: Some(proto::apply_request::El::Room(grpc_room)),
        }
    }
}
...
impl Into<proto::Member> for Member {
    fn into(self) -> proto::Member {
        ...
        proto::Member {
            ...
            reconnect_timeout: self.reconnect_timeout.map(|d| d.try_into().unwrap()),
        }
    }
}

impl Member {
    pub fn build_request<T>(self, url: T) -> proto::CreateRequest
    where
        T: Into<String>,
    {
        proto::CreateRequest {
...
impl WebRtcPlayEndpoint {
    pub fn build_request<T: Into<String>>(self, url: T) -> proto::CreateRequest {
        proto::CreateRequest {
...
impl Into<proto::WebRtcPublishEndpoint> for WebRtcPublishEndpoint {
    fn into(self) -> proto::WebRtcPublishEndpoint {
        use proto::web_rtc_publish_endpoint::{AudioSettings, PublishPolicy, VideoSettings};
...
/// ```
pub fn pub_sub_room_req(room_id: &str, is_sfu: bool) -> proto::CreateRequest {
    RoomBuilder::default()
            ...
            MemberBuilder::default()
                ...
                .add_endpoint(
                    WebRtcPublishEndpointBuilder::default()
                        .id("publish")
                        .p2p_mode(if is_sfu {
                            proto::web_rtc_publish_endpoint::P2p::Never
                        } else {
                            proto::web_rtc_publish_endpoint::P2p::Always
                        })
                        .build()
                        .unwrap(),
                )
            ...
            MemberBuilder::default()
                ...
                .add_endpoint(
                    WebRtcPlayEndpointBuilder::default()
                        .id("play")
                        .src(format!("local://{room_id}/publisher/publish"))
                        .build()
                        .unwrap(),
                )
                .build()
        ...
        .add_member(
            MemberBuilder::default()
                ...
                .add_endpoint(
                    WebRtcPlayEndpointBuilder::default()
                        .id("play")
                        .src(format!("local://{room_id}/bob/publish"))
                        .build()
                        .unwrap(),
                )
                .build()
        ...
        .add_member(
            MemberBuilder::default()
                ...
                .add_endpoint(
                    WebRtcPlayEndpointBuilder::default()
                        .id("play")
                        .src(format!("local://{room_id}/alice/publish"))
                        .build()
                        .unwrap(),
                )
                .build()
